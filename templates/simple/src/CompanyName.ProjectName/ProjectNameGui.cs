...
internal sealed partial class ProjectNameGui : IGuiTool
{
    ...
    = new(
        name: $"{nameof(ProjectNameGui)}.{nameof(outputFilePath)}",
        defaultValue: "");
    private static readonly SettingDefinition<SplitType> splitType
    = new(
        name: $"{nameof(ProjectNameGui)}.{nameof(splitType)}",
        defaultValue: SplitType.Size);
    private static readonly SettingDefinition<int> size
  = new(
      name: $"{nameof(ProjectNameGui)}.{nameof(size)}",
      defaultValue: 1024 * 1024 * 2);
    private static readonly SettingDefinition<int> line
  = new(
      name: $"{nameof(ProjectNameGui)}.{nameof(line)}",
      defaultValue: 1024);
    [ImportingConstructor]
    public ProjectNameGui(ISettingsProvider settingsProvider, IFileStorage fileStorage)
    {
    ...
    private readonly IUIInfoBar _infoBar = InfoBar(nameof(_infoBar)).Informational()
                        .Title(ProjectName.SplittedTitle)
                        .Description(ProjectName.SplittedDescrtion)
                       ;
    private readonly IUIInfoBar _errorBar = InfoBar(nameof(_errorBar))
                      .Title(ProjectName.ErrorInfoTitle)
                      .Description(ProjectName.ErrorInfoDescription)
                     ;
            ...
            new(Stack()
                ...
                .WithChildren(
                    ...
                    SelectDropDownList()
                        ...
                        .WithItems(
                            Item(ProjectName.SplitType_Size, SplitType.Size),
                            Item(ProjectName.SplitType_Line, SplitType.Line)
                            )
    ...
    {
        _settingsProvider.SetSetting(ProjectNameGui.outputFilePath, arg);
    }
